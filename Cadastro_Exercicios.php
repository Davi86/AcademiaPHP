...
<html lang="pt-br">
<head>
	<title>Cadastrar Exercícios</title>
	<meta charset="utf-8">
			...
			<div class="col s10">
				<div class="card amber" style="padding: 2px;">
					<div class="card-content grey darken-4">
							<span class="card-title center white-text">CADASTRO DOS EXERCÍCIOS</span>
					</div>
				</div>
			</div>
		</div>
		<form method="post" action="Exercicios_Funcao.php">
		<div class="row">
			<div class="col s1"></div>
			<div class="col s5">
				...
				<div class="card grey darken-4">
					<div class="card-content">
						<div class="input-field">
							...
							<select type="text" name="grupo" required>
								<option class="white-text">Escolha o Grupo Muscular</option>
								<?php 
									$sql = "SELECT nome FROM grupos";
									$grupos = mysqli_query($conexao_bd,$sql);
									while ($nome = mysqli_fetch_assoc($grupos)) 
									{ ?>
									<option class="white-text"><?php echo $nome['nome']; ?></option>
								<?php } ?>
							</select>
						</div>
					</div>
					...
					<div class="card-content">
						<div class="card-content">
							<div class="input-field">
								...
								<textarea name="obs" id="autocomplete4" class="autocomplete white-text"></textarea>
								<label for="autocomplete4" class="white-text">Observação</label>
							</div>
						</div>
					</div>
				</div>
